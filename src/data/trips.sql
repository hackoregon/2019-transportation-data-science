...
SELECT to_timestamp(opd_date, 'DDMONYYYY:HH24:MI:SS') AS date_stamp, event_no AS event_no_trip,
  vehicle_id, meters, act_dep_time, nom_dep_time, nom_end_time, act_end_time,
...
\echo
\echo indexing
CREATE INDEX ON trips (date_stamp);
CREATE INDEX ON trips (event_no_trip);
CREATE INDEX ON trips (vehicle_id);
CREATE INDEX ON trips (line_id, pattern_direction);
\echo
\echo primary key
ALTER TABLE trips ADD PRIMARY KEY (event_no_trip);
\echo
\echo vacuuming
